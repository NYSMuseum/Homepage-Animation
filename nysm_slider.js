(function (cjs, an) {

var p; // shortcut to reference prototypes
var lib={};var ss={};var img={};
lib.ssMetadata = [];


(lib.AnMovieClip = function(){
	this.actionFrames = [];
	this.ignorePause = false;
	this.gotoAndPlay = function(positionOrLabel){
		cjs.MovieClip.prototype.gotoAndPlay.call(this,positionOrLabel);
	}
	this.play = function(){
		cjs.MovieClip.prototype.play.call(this);
	}
	this.gotoAndStop = function(positionOrLabel){
		cjs.MovieClip.prototype.gotoAndStop.call(this,positionOrLabel);
	}
	this.stop = function(){
		cjs.MovieClip.prototype.stop.call(this);
	}
}).prototype = p = new cjs.MovieClip();
// symbols:



(lib.Asset131x = function() {
	this.initialize(img.Asset131x);
}).prototype = p = new cjs.Bitmap();
p.nominalBounds = new cjs.Rectangle(0,0,688,510);


(lib.Asset201x = function() {
	this.initialize(img.Asset201x);
}).prototype = p = new cjs.Bitmap();
p.nominalBounds = new cjs.Rectangle(0,0,1601,477);


(lib.bluefacade = function() {
	this.initialize(img.bluefacade);
}).prototype = p = new cjs.Bitmap();
p.nominalBounds = new cjs.Rectangle(0,0,1050,420);


(lib.BorealBog_Mural_SAMPLEkg = function() {
	this.initialize(img.BorealBog_Mural_SAMPLEkg);
}).prototype = p = new cjs.Bitmap();
p.nominalBounds = new cjs.Rectangle(0,0,1100,407);


(lib.capturedlobbylcd = function() {
	this.initialize(img.capturedlobbylcd);
}).prototype = p = new cjs.Bitmap();
p.nominalBounds = new cjs.Rectangle(0,0,400,400);


(lib.circle = function() {
	this.initialize(img.circle);
}).prototype = p = new cjs.Bitmap();
p.nominalBounds = new cjs.Rectangle(0,0,1000,400);


(lib.douglas = function() {
	this.initialize(img.douglas);
}).prototype = p = new cjs.Bitmap();
p.nominalBounds = new cjs.Rectangle(0,0,351,400);


(lib.FORTOframesize = function() {
	this.initialize(img.FORTOframesize);
}).prototype = p = new cjs.Bitmap();
p.nominalBounds = new cjs.Rectangle(0,0,1001,401);


(lib.fortorangealbanymap = function() {
	this.initialize(img.fortorangealbanymap);
}).prototype = p = new cjs.Bitmap();
p.nominalBounds = new cjs.Rectangle(0,0,1001,401);


(lib.fortorange = function() {
	this.initialize(img.fortorange);
}).prototype = p = new cjs.Bitmap();
p.nominalBounds = new cjs.Rectangle(0,0,1001,401);


(lib.henryjohnson = function() {
	this.initialize(img.henryjohnson);
}).prototype = p = new cjs.Bitmap();
p.nominalBounds = new cjs.Rectangle(0,0,320,400);


(lib.iceagestitle = function() {
	this.initialize(img.iceagestitle);
}).prototype = p = new cjs.Bitmap();
p.nominalBounds = new cjs.Rectangle(0,0,321,300);


(lib.shirley = function() {
	this.initialize(img.shirley);
}).prototype = p = new cjs.Bitmap();
p.nominalBounds = new cjs.Rectangle(0,0,268,400);


(lib.title01 = function() {
	this.initialize(img.title01);
}).prototype = p = new cjs.Bitmap();
p.nominalBounds = new cjs.Rectangle(0,0,797,115);


(lib.title02 = function() {
	this.initialize(img.title02);
}).prototype = p = new cjs.Bitmap();
p.nominalBounds = new cjs.Rectangle(0,0,489,131);


(lib.tubman = function() {
	this.initialize(img.tubman);
}).prototype = p = new cjs.Bitmap();
p.nominalBounds = new cjs.Rectangle(0,0,300,400);


(lib.underline = function() {
	this.initialize(img.underline);
}).prototype = p = new cjs.Bitmap();
p.nominalBounds = new cjs.Rectangle(0,0,1000,400);


(lib.wereopentextpngcopy = function() {
	this.initialize(img.wereopentextpngcopy);
}).prototype = p = new cjs.Bitmap();
p.nominalBounds = new cjs.Rectangle(0,0,1000,400);// helper functions:

function mc_symbol_clone() {
	var clone = this._cloneProps(new this.constructor(this.mode, this.startPosition, this.loop, this.reversed));
	clone.gotoAndStop(this.currentFrame);
	clone.paused = this.paused;
	clone.framerate = this.framerate;
	return clone;
}

function getMCSymbolPrototype(symbol, nominalBounds, frameBounds) {
	var prototype = cjs.extend(symbol, cjs.MovieClip);
	prototype.clone = mc_symbol_clone;
	prototype.nominalBounds = nominalBounds;
	prototype.frameBounds = frameBounds;
	return prototype;
	}


(lib.Tween109 = function(mode,startPosition,loop,reversed) {
if (loop == null) { loop = true; }
if (reversed == null) { reversed = false; }
	var props = new Object();
	props.mode = mode;
	props.startPosition = startPosition;
	props.labels = {};
	props.loop = loop;
	props.reversed = reversed;
	cjs.MovieClip.apply(this,[props]);

	// Layer_1
	this.instance = new lib.tubman();
	this.instance.setTransform(-150,-200);

	this.timeline.addTween(cjs.Tween.get(this.instance).wait(1));

	this._renderFirstFrame();

}).prototype = p = new cjs.MovieClip();
p.nominalBounds = new cjs.Rectangle(-150,-200,300,400);


(lib.Tween108 = function(mode,startPosition,loop,reversed) {
if (loop == null) { loop = true; }
if (reversed == null) { reversed = false; }
	var props = new Object();
	props.mode = mode;
	props.startPosition = startPosition;
	props.labels = {};
	props.loop = loop;
	props.reversed = reversed;
	cjs.MovieClip.apply(this,[props]);

	// Layer_1
	this.instance = new lib.tubman();
	this.instance.setTransform(-150,-200);

	this.timeline.addTween(cjs.Tween.get(this.instance).wait(1));

	this._renderFirstFrame();

}).prototype = p = new cjs.MovieClip();
p.nominalBounds = new cjs.Rectangle(-150,-200,300,400);


(lib.Tween107 = function(mode,startPosition,loop,reversed) {
if (loop == null) { loop = true; }
if (reversed == null) { reversed = false; }
	var props = new Object();
	props.mode = mode;
	props.startPosition = startPosition;
	props.labels = {};
	props.loop = loop;
	props.reversed = reversed;
	cjs.MovieClip.apply(this,[props]);

	// Layer_1
	this.instance = new lib.henryjohnson();
	this.instance.setTransform(-160,-200);

	this.timeline.addTween(cjs.Tween.get(this.instance).wait(1));

	this._renderFirstFrame();

}).prototype = p = new cjs.MovieClip();
p.nominalBounds = new cjs.Rectangle(-160,-200,320,400);


(lib.Tween106 = function(mode,startPosition,loop,reversed) {
if (loop == null) { loop = true; }
if (reversed == null) { reversed = false; }
	var props = new Object();
	props.mode = mode;
	props.startPosition = startPosition;
	props.labels = {};
	props.loop = loop;
	props.reversed = reversed;
	cjs.MovieClip.apply(this,[props]);

	// Layer_1
	this.instance = new lib.henryjohnson();
	this.instance.setTransform(-160,-200);

	this.timeline.addTween(cjs.Tween.get(this.instance).wait(1));

	this._renderFirstFrame();

}).prototype = p = new cjs.MovieClip();
p.nominalBounds = new cjs.Rectangle(-160,-200,320,400);


(lib.Tween105 = function(mode,startPosition,loop,reversed) {
if (loop == null) { loop = true; }
if (reversed == null) { reversed = false; }
	var props = new Object();
	props.mode = mode;
	props.startPosition = startPosition;
	props.labels = {};
	props.loop = loop;
	props.reversed = reversed;
	cjs.MovieClip.apply(this,[props]);

	// Layer_1
	this.instance = new lib.shirley();
	this.instance.setTransform(-134,-200);

	this.timeline.addTween(cjs.Tween.get(this.instance).wait(1));

	this._renderFirstFrame();

}).prototype = p = new cjs.MovieClip();
p.nominalBounds = new cjs.Rectangle(-134,-200,268,400);


(lib.Tween104 = function(mode,startPosition,loop,reversed) {
if (loop == null) { loop = true; }
if (reversed == null) { reversed = false; }
	var props = new Object();
	props.mode = mode;
	props.startPosition = startPosition;
	props.labels = {};
	props.loop = loop;
	props.reversed = reversed;
	cjs.MovieClip.apply(this,[props]);

	// Layer_1
	this.instance = new lib.shirley();
	this.instance.setTransform(-134,-200);

	this.timeline.addTween(cjs.Tween.get(this.instance).wait(1));

	this._renderFirstFrame();

}).prototype = p = new cjs.MovieClip();
p.nominalBounds = new cjs.Rectangle(-134,-200,268,400);


(lib.Tween103 = function(mode,startPosition,loop,reversed) {
if (loop == null) { loop = true; }
if (reversed == null) { reversed = false; }
	var props = new Object();
	props.mode = mode;
	props.startPosition = startPosition;
	props.labels = {};
	props.loop = loop;
	props.reversed = reversed;
	cjs.MovieClip.apply(this,[props]);

	// Layer_1
	this.instance = new lib.douglas();
	this.instance.setTransform(-175.5,-200);

	this.timeline.addTween(cjs.Tween.get(this.instance).wait(1));

	this._renderFirstFrame();

}).prototype = p = new cjs.MovieClip();
p.nominalBounds = new cjs.Rectangle(-175.5,-200,351,400);


(lib.Tween102 = function(mode,startPosition,loop,reversed) {
if (loop == null) { loop = true; }
if (reversed == null) { reversed = false; }
	var props = new Object();
	props.mode = mode;
	props.startPosition = startPosition;
	props.labels = {};
	props.loop = loop;
	props.reversed = reversed;
	cjs.MovieClip.apply(this,[props]);

	// Layer_1
	this.instance = new lib.douglas();
	this.instance.setTransform(-175.5,-200);

	this.timeline.addTween(cjs.Tween.get(this.instance).wait(1));

	this._renderFirstFrame();

}).prototype = p = new cjs.MovieClip();
p.nominalBounds = new cjs.Rectangle(-175.5,-200,351,400);


(lib.Tween96 = function(mode,startPosition,loop,reversed) {
if (loop == null) { loop = true; }
if (reversed == null) { reversed = false; }
	var props = new Object();
	props.mode = mode;
	props.startPosition = startPosition;
	props.labels = {};
	props.loop = loop;
	props.reversed = reversed;
	cjs.MovieClip.apply(this,[props]);

	// Layer_1
	this.shape = new cjs.Shape();
	this.shape.graphics.f("#000000").s().p("EgmhAgnMAAAhBNMBNDAAAMAAABBNg");
	this.shape.setTransform(0.025,0.025);

	this.timeline.addTween(cjs.Tween.get(this.shape).wait(1));

	this._renderFirstFrame();

}).prototype = p = new cjs.MovieClip();
p.nominalBounds = new cjs.Rectangle(-246.6,-208.7,493.29999999999995,417.5);


(lib.Tween95 = function(mode,startPosition,loop,reversed) {
if (loop == null) { loop = true; }
if (reversed == null) { reversed = false; }
	var props = new Object();
	props.mode = mode;
	props.startPosition = startPosition;
	props.labels = {};
	props.loop = loop;
	props.reversed = reversed;
	cjs.MovieClip.apply(this,[props]);

	// Layer_1
	this.shape = new cjs.Shape();
	this.shape.graphics.f("#000000").s().p("EgmhAgnMAAAhBNMBNDAAAMAAABBNg");
	this.shape.setTransform(0.025,0.025);

	this.timeline.addTween(cjs.Tween.get(this.shape).wait(1));

	this._renderFirstFrame();

}).prototype = p = new cjs.MovieClip();
p.nominalBounds = new cjs.Rectangle(-246.6,-208.7,493.29999999999995,417.5);


(lib.Tween92 = function(mode,startPosition,loop,reversed) {
if (loop == null) { loop = true; }
if (reversed == null) { reversed = false; }
	var props = new Object();
	props.mode = mode;
	props.startPosition = startPosition;
	props.labels = {};
	props.loop = loop;
	props.reversed = reversed;
	cjs.MovieClip.apply(this,[props]);

	// Layer_1
	this.instance = new lib.Asset131x();
	this.instance.setTransform(-73.55,-54.55,0.2139,0.2139);

	this.timeline.addTween(cjs.Tween.get(this.instance).wait(1));

	this._renderFirstFrame();

}).prototype = p = new cjs.MovieClip();
p.nominalBounds = new cjs.Rectangle(-73.5,-54.5,147.1,109.1);


(lib.Tween91 = function(mode,startPosition,loop,reversed) {
if (loop == null) { loop = true; }
if (reversed == null) { reversed = false; }
	var props = new Object();
	props.mode = mode;
	props.startPosition = startPosition;
	props.labels = {};
	props.loop = loop;
	props.reversed = reversed;
	cjs.MovieClip.apply(this,[props]);

	// Layer_1
	this.instance = new lib.Asset131x();
	this.instance.setTransform(-73.55,-54.55,0.2139,0.2139);

	this.timeline.addTween(cjs.Tween.get(this.instance).wait(1));

	this._renderFirstFrame();

}).prototype = p = new cjs.MovieClip();
p.nominalBounds = new cjs.Rectangle(-73.5,-54.5,147.1,109.1);


(lib.Tween90 = function(mode,startPosition,loop,reversed) {
if (loop == null) { loop = true; }
if (reversed == null) { reversed = false; }
	var props = new Object();
	props.mode = mode;
	props.startPosition = startPosition;
	props.labels = {};
	props.loop = loop;
	props.reversed = reversed;
	cjs.MovieClip.apply(this,[props]);

	// Layer_1
	this.instance = new lib.Asset201x();
	this.instance.setTransform(-227.75,-67.85,0.2845,0.2845);

	this.timeline.addTween(cjs.Tween.get(this.instance).wait(1));

	this._renderFirstFrame();

}).prototype = p = new cjs.MovieClip();
p.nominalBounds = new cjs.Rectangle(-227.7,-67.8,455.5,135.7);


(lib.Tween89 = function(mode,startPosition,loop,reversed) {
if (loop == null) { loop = true; }
if (reversed == null) { reversed = false; }
	var props = new Object();
	props.mode = mode;
	props.startPosition = startPosition;
	props.labels = {};
	props.loop = loop;
	props.reversed = reversed;
	cjs.MovieClip.apply(this,[props]);

	// Layer_1
	this.instance = new lib.Asset201x();
	this.instance.setTransform(-227.75,-67.85,0.2845,0.2845);

	this.timeline.addTween(cjs.Tween.get(this.instance).wait(1));

	this._renderFirstFrame();

}).prototype = p = new cjs.MovieClip();
p.nominalBounds = new cjs.Rectangle(-227.7,-67.8,455.5,135.7);


(lib.Tween88 = function(mode,startPosition,loop,reversed) {
if (loop == null) { loop = true; }
if (reversed == null) { reversed = false; }
	var props = new Object();
	props.mode = mode;
	props.startPosition = startPosition;
	props.labels = {};
	props.loop = loop;
	props.reversed = reversed;
	cjs.MovieClip.apply(this,[props]);

	// Layer_1
	this.instance = new lib.capturedlobbylcd();
	this.instance.setTransform(-200,-200);

	this.timeline.addTween(cjs.Tween.get(this.instance).wait(1));

	this._renderFirstFrame();

}).prototype = p = new cjs.MovieClip();
p.nominalBounds = new cjs.Rectangle(-200,-200,400,400);


(lib.Tween87 = function(mode,startPosition,loop,reversed) {
if (loop == null) { loop = true; }
if (reversed == null) { reversed = false; }
	var props = new Object();
	props.mode = mode;
	props.startPosition = startPosition;
	props.labels = {};
	props.loop = loop;
	props.reversed = reversed;
	cjs.MovieClip.apply(this,[props]);

	// Layer_1
	this.instance = new lib.capturedlobbylcd();
	this.instance.setTransform(-200,-200);

	this.timeline.addTween(cjs.Tween.get(this.instance).wait(1));

	this._renderFirstFrame();

}).prototype = p = new cjs.MovieClip();
p.nominalBounds = new cjs.Rectangle(-200,-200,400,400);


(lib.Tween86 = function(mode,startPosition,loop,reversed) {
if (loop == null) { loop = true; }
if (reversed == null) { reversed = false; }
	var props = new Object();
	props.mode = mode;
	props.startPosition = startPosition;
	props.labels = {};
	props.loop = loop;
	props.reversed = reversed;
	cjs.MovieClip.apply(this,[props]);

	// Layer_1
	this.instance = new lib.underline();
	this.instance.setTransform(-500,-200);

	this.timeline.addTween(cjs.Tween.get(this.instance).wait(1));

	this._renderFirstFrame();

}).prototype = p = new cjs.MovieClip();
p.nominalBounds = new cjs.Rectangle(-500,-200,1000,400);


(lib.Tween76 = function(mode,startPosition,loop,reversed) {
if (loop == null) { loop = true; }
if (reversed == null) { reversed = false; }
	var props = new Object();
	props.mode = mode;
	props.startPosition = startPosition;
	props.labels = {};
	props.loop = loop;
	props.reversed = reversed;
	cjs.MovieClip.apply(this,[props]);

	// Layer_1
	this.instance = new lib.bluefacade();
	this.instance.setTransform(-525,-210);

	this.timeline.addTween(cjs.Tween.get(this.instance).wait(1));

	this._renderFirstFrame();

}).prototype = p = new cjs.MovieClip();
p.nominalBounds = new cjs.Rectangle(-525,-210,1050,420);


(lib.Tween75 = function(mode,startPosition,loop,reversed) {
if (loop == null) { loop = true; }
if (reversed == null) { reversed = false; }
	var props = new Object();
	props.mode = mode;
	props.startPosition = startPosition;
	props.labels = {};
	props.loop = loop;
	props.reversed = reversed;
	cjs.MovieClip.apply(this,[props]);

	// Layer_1
	this.instance = new lib.bluefacade();
	this.instance.setTransform(-525,-210);

	this.timeline.addTween(cjs.Tween.get(this.instance).wait(1));

	this._renderFirstFrame();

}).prototype = p = new cjs.MovieClip();
p.nominalBounds = new cjs.Rectangle(-525,-210,1050,420);


(lib.Tween72 = function(mode,startPosition,loop,reversed) {
if (loop == null) { loop = true; }
if (reversed == null) { reversed = false; }
	var props = new Object();
	props.mode = mode;
	props.startPosition = startPosition;
	props.labels = {};
	props.loop = loop;
	props.reversed = reversed;
	cjs.MovieClip.apply(this,[props]);

	// Layer_1
	this.instance = new lib.title01();
	this.instance.setTransform(-398.5,-57.5);

	this.timeline.addTween(cjs.Tween.get(this.instance).wait(1));

	this._renderFirstFrame();

}).prototype = p = new cjs.MovieClip();
p.nominalBounds = new cjs.Rectangle(-398.5,-57.5,797,115);


(lib.Tween71 = function(mode,startPosition,loop,reversed) {
if (loop == null) { loop = true; }
if (reversed == null) { reversed = false; }
	var props = new Object();
	props.mode = mode;
	props.startPosition = startPosition;
	props.labels = {};
	props.loop = loop;
	props.reversed = reversed;
	cjs.MovieClip.apply(this,[props]);

	// Layer_1
	this.instance = new lib.title01();
	this.instance.setTransform(-398.5,-57.5);

	this.timeline.addTween(cjs.Tween.get(this.instance).wait(1));

	this._renderFirstFrame();

}).prototype = p = new cjs.MovieClip();
p.nominalBounds = new cjs.Rectangle(-398.5,-57.5,797,115);


(lib.Tween22 = function(mode,startPosition,loop,reversed) {
if (loop == null) { loop = true; }
if (reversed == null) { reversed = false; }
	var props = new Object();
	props.mode = mode;
	props.startPosition = startPosition;
	props.labels = {};
	props.loop = loop;
	props.reversed = reversed;
	cjs.MovieClip.apply(this,[props]);

	// Layer 1
	this.instance = new lib.iceagestitle();
	this.instance.setTransform(-160.5,-150);

	this.timeline.addTween(cjs.Tween.get(this.instance).wait(1));

	this._renderFirstFrame();

}).prototype = p = new cjs.MovieClip();
p.nominalBounds = new cjs.Rectangle(-160.5,-150,321,300);


(lib.Tween17 = function(mode,startPosition,loop,reversed) {
if (loop == null) { loop = true; }
if (reversed == null) { reversed = false; }
	var props = new Object();
	props.mode = mode;
	props.startPosition = startPosition;
	props.labels = {};
	props.loop = loop;
	props.reversed = reversed;
	cjs.MovieClip.apply(this,[props]);

	// Layer 1
	this.instance = new lib.BorealBog_Mural_SAMPLEkg();
	this.instance.setTransform(-550,-203.5);

	this.timeline.addTween(cjs.Tween.get(this.instance).wait(1));

	this._renderFirstFrame();

}).prototype = p = new cjs.MovieClip();
p.nominalBounds = new cjs.Rectangle(-550,-203.5,1100,407);


(lib.Symbol14 = function(mode,startPosition,loop,reversed) {
if (loop == null) { loop = true; }
if (reversed == null) { reversed = false; }
	var props = new Object();
	props.mode = mode;
	props.startPosition = startPosition;
	props.labels = {};
	props.loop = loop;
	props.reversed = reversed;
	cjs.MovieClip.apply(this,[props]);

	// Layer_1
	this.instance = new lib.wereopentextpngcopy();

	this.timeline.addTween(cjs.Tween.get(this.instance).wait(1));

	this._renderFirstFrame();

}).prototype = getMCSymbolPrototype(lib.Symbol14, new cjs.Rectangle(0,0,1000,400), null);


(lib.Symbol13 = function(mode,startPosition,loop,reversed) {
if (loop == null) { loop = true; }
if (reversed == null) { reversed = false; }
	var props = new Object();
	props.mode = mode;
	props.startPosition = startPosition;
	props.labels = {};
	props.loop = loop;
	props.reversed = reversed;
	cjs.MovieClip.apply(this,[props]);

	// Layer_1
	this.instance = new lib.circle();

	this.timeline.addTween(cjs.Tween.get(this.instance).wait(1));

	this._renderFirstFrame();

}).prototype = getMCSymbolPrototype(lib.Symbol13, new cjs.Rectangle(0,0,1000,400), null);


(lib.Forttitle02 = function(mode,startPosition,loop,reversed) {
if (loop == null) { loop = true; }
if (reversed == null) { reversed = false; }
	var props = new Object();
	props.mode = mode;
	props.startPosition = startPosition;
	props.labels = {};
	props.loop = loop;
	props.reversed = reversed;
	cjs.MovieClip.apply(this,[props]);

	// Layer 1
	this.instance = new lib.title02();

	this.timeline.addTween(cjs.Tween.get(this.instance).wait(1));

	this._renderFirstFrame();

}).prototype = p = new cjs.MovieClip();
p.nominalBounds = new cjs.Rectangle(0,0,489,131);


(lib.wereopen = function(mode,startPosition,loop,reversed) {
if (loop == null) { loop = true; }
if (reversed == null) { reversed = false; }
	var props = new Object();
	props.mode = mode;
	props.startPosition = startPosition;
	props.labels = {};
	props.loop = loop;
	props.reversed = reversed;
	cjs.MovieClip.apply(this,[props]);

	// timeline functions:
	this.frame_0 = function() {
		this.stop();
	}
	this.frame_194 = function() {
		this.getStage().getChildAt(0).changeSlide(slideNum);
	}

	// actions tween:
	this.timeline.addTween(cjs.Tween.get(this).call(this.frame_0).wait(194).call(this.frame_194).wait(31));

	// underline
	this.instance = new lib.Tween86("synched",0);
	this.instance.setTransform(499,200,0.001,1);
	this.instance._off = true;

	this.timeline.addTween(cjs.Tween.get(this.instance).wait(40).to({_off:false},0).to({scaleX:1},20).to({startPosition:0},79).to({startPosition:0},85).wait(1));

	// we'reopen
	this.instance_1 = new lib.Symbol14();
	this.instance_1.setTransform(499,201,1,1,0,0,0,500,200);
	this.instance_1.alpha = 0;
	this.instance_1._off = true;

	this.timeline.addTween(cjs.Tween.get(this.instance_1).wait(31).to({_off:false},0).wait(1).to({alpha:0.0588},0).wait(1).to({alpha:0.1176},0).wait(1).to({alpha:0.1765},0).wait(1).to({alpha:0.2353},0).wait(1).to({alpha:0.2941},0).wait(1).to({alpha:0.3529},0).wait(1).to({alpha:0.4118},0).wait(1).to({alpha:0.4706},0).wait(1).to({alpha:0.5294},0).wait(1).to({alpha:0.5882},0).wait(1).to({alpha:0.6471},0).wait(1).to({alpha:0.7059},0).wait(1).to({alpha:0.7647},0).wait(1).to({alpha:0.8235},0).wait(1).to({alpha:0.8824},0).wait(179));

	// Circle
	this.instance_2 = new lib.Symbol13();
	this.instance_2.setTransform(500,200,1,1,0,0,0,500,200);
	this.instance_2.alpha = 0;

	this.timeline.addTween(cjs.Tween.get(this.instance_2).wait(13).to({alpha:0.0625},0).wait(1).to({alpha:0.125},0).wait(1).to({alpha:0.1875},0).wait(1).to({alpha:0.25},0).wait(1).to({alpha:0.3125},0).wait(1).to({alpha:0.375},0).wait(1).to({alpha:0.4375},0).wait(1).to({alpha:0.5},0).wait(1).to({alpha:0.5625},0).wait(1).to({alpha:0.625},0).wait(1).to({alpha:0.6875},0).wait(1).to({alpha:0.75},0).wait(1).to({alpha:0.8125},0).wait(1).to({alpha:0.875},0).wait(1).to({alpha:0.9375},0).wait(1).to({alpha:1},0).wait(197));

	// Layer_1
	this.instance_3 = new lib.Tween75("synched",0);
	this.instance_3.setTransform(499,200,0.9524,0.9524);

	this.instance_4 = new lib.Tween76("synched",0);
	this.instance_4.setTransform(499,200);

	this.timeline.addTween(cjs.Tween.get({}).to({state:[{t:this.instance_3}]}).to({state:[{t:this.instance_4}]},140).to({state:[{t:this.instance_4}]},84).wait(1));
	this.timeline.addTween(cjs.Tween.get(this.instance_3).to({_off:true,scaleX:1,scaleY:1},140).wait(85));

	this._renderFirstFrame();

}).prototype = p = new cjs.MovieClip();
p.nominalBounds = new cjs.Rectangle(-26,-10,1050,420);


(lib.votesforwomen = function(mode,startPosition,loop,reversed) {
if (loop == null) { loop = false; }
if (reversed == null) { reversed = false; }
	var props = new Object();
	props.mode = mode;
	props.startPosition = startPosition;
	props.labels = {};
	props.loop = loop;
	props.reversed = reversed;
	cjs.MovieClip.apply(this,[props]);

	// timeline functions:
	this.frame_0 = function() {
		this.stop();
	}
	this.frame_202 = function() {
		this.getStage().getChildAt(0).changeSlide(slideNum);
	}

	// actions tween:
	this.timeline.addTween(cjs.Tween.get(this).call(this.frame_0).wait(202).call(this.frame_202).wait(23));

	// title-fortorange
	this.instance = new lib.Forttitle02("synched",0);
	this.instance.setTransform(164.5,-78.5,1,1,0,0,0,244.5,65.5);
	this.instance.alpha = 0;
	this.instance._off = true;

	this.timeline.addTween(cjs.Tween.get(this.instance).wait(94).to({_off:false},0).to({alpha:1},19).wait(112));

	// Layer_2
	this.instance_1 = new lib.Tween71("synched",0);
	this.instance_1.setTransform(-32.5,-144.5);
	this.instance_1.alpha = 0;
	this.instance_1._off = true;

	this.instance_2 = new lib.Tween72("synched",0);
	this.instance_2.setTransform(-32.5,-144.5);

	this.timeline.addTween(cjs.Tween.get({}).to({state:[]}).to({state:[{t:this.instance_1}]},77).to({state:[{t:this.instance_2}]},21).wait(127));
	this.timeline.addTween(cjs.Tween.get(this.instance_1).wait(77).to({_off:false},0).to({_off:true,alpha:1},21).wait(127));

	// fort (mask)
	var mask = new cjs.Shape();
	mask._off = true;
	var mask_graphics_0 = new cjs.Graphics().p("");
	var mask_graphics_10 = new cjs.Graphics().p("");
	var mask_graphics_11 = new cjs.Graphics().p("");
	var mask_graphics_12 = new cjs.Graphics().p("");
	var mask_graphics_13 = new cjs.Graphics().p("");
	var mask_graphics_14 = new cjs.Graphics().p("");
	var mask_graphics_15 = new cjs.Graphics().p("");
	var mask_graphics_16 = new cjs.Graphics().p("");
	var mask_graphics_17 = new cjs.Graphics().p("");
	var mask_graphics_18 = new cjs.Graphics().p("");
	var mask_graphics_19 = new cjs.Graphics().p("");
	var mask_graphics_20 = new cjs.Graphics().p("");
	var mask_graphics_21 = new cjs.Graphics().p("");
	var mask_graphics_22 = new cjs.Graphics().p("");
	var mask_graphics_23 = new cjs.Graphics().p("");
	var mask_graphics_24 = new cjs.Graphics().p("");
	var mask_graphics_25 = new cjs.Graphics().p("");
	var mask_graphics_26 = new cjs.Graphics().p("");
	var mask_graphics_27 = new cjs.Graphics().p("");
	var mask_graphics_28 = new cjs.Graphics().p("");
	var mask_graphics_29 = new cjs.Graphics().p("");
	var mask_graphics_30 = new cjs.Graphics().p("");
	var mask_graphics_31 = new cjs.Graphics().p("");
	var mask_graphics_32 = new cjs.Graphics().p("");
	var mask_graphics_33 = new cjs.Graphics().p("");
	var mask_graphics_34 = new cjs.Graphics().p("");
	var mask_graphics_35 = new cjs.Graphics().p("");
	var mask_graphics_36 = new cjs.Graphics().p("");
	var mask_graphics_37 = new cjs.Graphics().p("");
	var mask_graphics_38 = new cjs.Graphics().p("");
	var mask_graphics_39 = new cjs.Graphics().p("");
	var mask_graphics_40 = new cjs.Graphics().p("");
	var mask_graphics_41 = new cjs.Graphics().p("");
	var mask_graphics_42 = new cjs.Graphics().p("");
	var mask_graphics_43 = new cjs.Graphics().p("");
	var mask_graphics_44 = new cjs.Graphics().p("");
	var mask_graphics_45 = new cjs.Graphics().p("");
	var mask_graphics_46 = new cjs.Graphics().p("");
	var mask_graphics_47 = new cjs.Graphics().p("");
	var mask_graphics_48 = new cjs.Graphics().p("");
	var mask_graphics_49 = new cjs.Graphics().p("");
	var mask_graphics_50 = new cjs.Graphics().p("");
	var mask_graphics_51 = new cjs.Graphics().p("");
	var mask_graphics_52 = new cjs.Graphics().p("");
	var mask_graphics_53 = new cjs.Graphics().p("");
	var mask_graphics_54 = new cjs.Graphics().p("");
	var mask_graphics_55 = new cjs.Graphics().p("");
	var mask_graphics_56 = new cjs.Graphics().p("");
	var mask_graphics_57 = new cjs.Graphics().p("");
	var mask_graphics_58 = new cjs.Graphics().p("");
	var mask_graphics_59 = new cjs.Graphics().p("");
	var mask_graphics_60 = new cjs.Graphics().p("");
	var mask_graphics_61 = new cjs.Graphics().p("");
	var mask_graphics_62 = new cjs.Graphics().p("");
	var mask_graphics_63 = new cjs.Graphics().p("");
	var mask_graphics_64 = new cjs.Graphics().p("");
	var mask_graphics_65 = new cjs.Graphics().p("");
	var mask_graphics_66 = new cjs.Graphics().p("");
	var mask_graphics_67 = new cjs.Graphics().p("");
	var mask_graphics_68 = new cjs.Graphics().p("");
	var mask_graphics_69 = new cjs.Graphics().p("");
	var mask_graphics_70 = new cjs.Graphics().p("");
	var mask_graphics_71 = new cjs.Graphics().p("");
	var mask_graphics_72 = new cjs.Graphics().p("");
	var mask_graphics_73 = new cjs.Graphics().p("");
	var mask_graphics_74 = new cjs.Graphics().p("");
	var mask_graphics_75 = new cjs.Graphics().p("");
	var mask_graphics_76 = new cjs.Graphics().p("");
	var mask_graphics_77 = new cjs.Graphics().p("");
	var mask_graphics_78 = new cjs.Graphics().p("");
	var mask_graphics_79 = new cjs.Graphics().p("");
	var mask_graphics_80 = new cjs.Graphics().p("");
	var mask_graphics_81 = new cjs.Graphics().p("");
	var mask_graphics_82 = new cjs.Graphics().p("");
	var mask_graphics_83 = new cjs.Graphics().p("");
	var mask_graphics_84 = new cjs.Graphics().p("");
	var mask_graphics_85 = new cjs.Graphics().p("");
	var mask_graphics_86 = new cjs.Graphics().p("");
	var mask_graphics_87 = new cjs.Graphics().p("");
	var mask_graphics_88 = new cjs.Graphics().p("");
	var mask_graphics_89 = new cjs.Graphics().p("");
	var mask_graphics_90 = new cjs.Graphics().p("");
	var mask_graphics_91 = new cjs.Graphics().p("");
	var mask_graphics_92 = new cjs.Graphics().p("");
	var mask_graphics_93 = new cjs.Graphics().p("");
	var mask_graphics_94 = new cjs.Graphics().p("");
	var mask_graphics_95 = new cjs.Graphics().p("");
	var mask_graphics_96 = new cjs.Graphics().p("");
	var mask_graphics_97 = new cjs.Graphics().p("");
	var mask_graphics_98 = new cjs.Graphics().p("");
	var mask_graphics_99 = new cjs.Graphics().p("");
	var mask_graphics_100 = new cjs.Graphics().p("");
	var mask_graphics_101 = new cjs.Graphics().p("");
	var mask_graphics_102 = new cjs.Graphics().p("");
	var mask_graphics_103 = new cjs.Graphics().p("");
	var mask_graphics_104 = new cjs.Graphics().p("");
	var mask_graphics_105 = new cjs.Graphics().p("");
	var mask_graphics_106 = new cjs.Graphics().p("");
	var mask_graphics_107 = new cjs.Graphics().p("");
	var mask_graphics_108 = new cjs.Graphics().p("");
	var mask_graphics_109 = new cjs.Graphics().p("");
	var mask_graphics_110 = new cjs.Graphics().p("");
	var mask_graphics_111 = new cjs.Graphics().p("");
	var mask_graphics_112 = new cjs.Graphics().p("");
	var mask_graphics_113 = new cjs.Graphics().p("");
	var mask_graphics_114 = new cjs.Graphics().p("");
	var mask_graphics_115 = new cjs.Graphics().p("");
	var mask_graphics_116 = new cjs.Graphics().p("");
	var mask_graphics_117 = new cjs.Graphics().p("");
	var mask_graphics_118 = new cjs.Graphics().p("");
	var mask_graphics_119 = new cjs.Graphics().p("");
	var mask_graphics_120 = new cjs.Graphics().p("");
	var mask_graphics_121 = new cjs.Graphics().p("");
	var mask_graphics_122 = new cjs.Graphics().p("");
	var mask_graphics_123 = new cjs.Graphics().p("");
	var mask_graphics_124 = new cjs.Graphics().p("");
	var mask_graphics_125 = new cjs.Graphics().p("");
	var mask_graphics_126 = new cjs.Graphics().p("");

	this.timeline.addTween(cjs.Tween.get(mask).to({graphics:mask_graphics_0,x:-54.892,y:21.0378}).wait(10).to({graphics:mask_graphics_10,x:-54.892,y:21.0378}).wait(1).to({graphics:mask_graphics_11,x:-58.1972,y:20.9371}).wait(1).to({graphics:mask_graphics_12,x:-61.4944,y:20.8363}).wait(1).to({graphics:mask_graphics_13,x:-64.7837,y:20.7354}).wait(1).to({graphics:mask_graphics_14,x:-68.0651,y:20.6343}).wait(1).to({graphics:mask_graphics_15,x:-71.1441,y:20.5331}).wait(1).to({graphics:mask_graphics_16,x:-70.3089,y:20.4317}).wait(1).to({graphics:mask_graphics_17,x:-69.4734,y:20.3302}).wait(1).to({graphics:mask_graphics_18,x:-68.6378,y:20.2286}).wait(1).to({graphics:mask_graphics_19,x:-67.8019,y:20.1268}).wait(1).to({graphics:mask_graphics_20,x:-66.9659,y:20.0249}).wait(1).to({graphics:mask_graphics_21,x:-66.1296,y:19.9229}).wait(1).to({graphics:mask_graphics_22,x:-65.2932,y:19.8207}).wait(1).to({graphics:mask_graphics_23,x:-64.4565,y:19.7184}).wait(1).to({graphics:mask_graphics_24,x:-63.6197,y:19.6159}).wait(1).to({graphics:mask_graphics_25,x:-62.7826,y:19.5133}).wait(1).to({graphics:mask_graphics_26,x:-61.9454,y:19.4105}).wait(1).to({graphics:mask_graphics_27,x:-61.1079,y:19.3076}).wait(1).to({graphics:mask_graphics_28,x:-60.2703,y:19.2045}).wait(1).to({graphics:mask_graphics_29,x:-59.4324,y:19.1013}).wait(1).to({graphics:mask_graphics_30,x:-58.5944,y:18.998}).wait(1).to({graphics:mask_graphics_31,x:-57.7562,y:18.8945}).wait(1).to({graphics:mask_graphics_32,x:-56.9178,y:18.7908}).wait(1).to({graphics:mask_graphics_33,x:-56.0792,y:18.687}).wait(1).to({graphics:mask_graphics_34,x:-55.2404,y:18.5831}).wait(1).to({graphics:mask_graphics_35,x:-54.4014,y:18.479}).wait(1).to({graphics:mask_graphics_36,x:-53.5622,y:18.3747}).wait(1).to({graphics:mask_graphics_37,x:-52.7229,y:18.2703}).wait(1).to({graphics:mask_graphics_38,x:-51.8834,y:18.1657}).wait(1).to({graphics:mask_graphics_39,x:-51.0436,y:18.061}).wait(1).to({graphics:mask_graphics_40,x:-50.2038,y:17.9561}).wait(1).to({graphics:mask_graphics_41,x:-49.3637,y:17.8511}).wait(1).to({graphics:mask_graphics_42,x:-48.5234,y:17.7459}).wait(1).to({graphics:mask_graphics_43,x:-47.683,y:17.6405}).wait(1).to({graphics:mask_graphics_44,x:-46.8424,y:17.535}).wait(1).to({graphics:mask_graphics_45,x:-46.0016,y:17.4293}).wait(1).to({graphics:mask_graphics_46,x:-45.1607,y:17.3234}).wait(1).to({graphics:mask_graphics_47,x:-44.3196,y:17.2174}).wait(1).to({graphics:mask_graphics_48,x:-43.4783,y:17.1112}).wait(1).to({graphics:mask_graphics_49,x:-42.6368,y:17.0049}).wait(1).to({graphics:mask_graphics_50,x:-41.7952,y:16.8984}).wait(1).to({graphics:mask_graphics_51,x:-40.9534,y:16.7917}).wait(1).to({graphics:mask_graphics_52,x:-40.1115,y:16.6849}).wait(1).to({graphics:mask_graphics_53,x:-39.2694,y:16.5778}).wait(1).to({graphics:mask_graphics_54,x:-38.4271,y:16.4707}).wait(1).to({graphics:mask_graphics_55,x:-37.5847,y:16.3633}).wait(1).to({graphics:mask_graphics_56,x:-36.7421,y:16.2558}).wait(1).to({graphics:mask_graphics_57,x:-35.8993,y:16.1481}).wait(1).to({graphics:mask_graphics_58,x:-35.0564,y:16.0402}).wait(1).to({graphics:mask_graphics_59,x:-34.2133,y:15.9321}).wait(1).to({graphics:mask_graphics_60,x:-33.3701,y:15.8239}).wait(1).to({graphics:mask_graphics_61,x:-32.5267,y:15.7155}).wait(1).to({graphics:mask_graphics_62,x:-31.6832,y:15.6069}).wait(1).to({graphics:mask_graphics_63,x:-30.8395,y:15.4981}).wait(1).to({graphics:mask_graphics_64,x:-29.9957,y:15.3892}).wait(1).to({graphics:mask_graphics_65,x:-29.1518,y:15.2801}).wait(1).to({graphics:mask_graphics_66,x:-28.3077,y:15.1708}).wait(1).to({graphics:mask_graphics_67,x:-27.4634,y:15.0613}).wait(1).to({graphics:mask_graphics_68,x:-26.619,y:14.9516}).wait(1).to({graphics:mask_graphics_69,x:-25.7745,y:14.8417}).wait(1).to({graphics:mask_graphics_70,x:-24.9298,y:14.7317}).wait(1).to({graphics:mask_graphics_71,x:-24.085,y:14.6214}).wait(1).to({graphics:mask_graphics_72,x:-23.24,y:14.511}).wait(1).to({graphics:mask_graphics_73,x:-22.3949,y:14.4004}).wait(1).to({graphics:mask_graphics_74,x:-21.5497,y:14.2896}).wait(1).to({graphics:mask_graphics_75,x:-20.7043,y:14.1786}).wait(1).to({graphics:mask_graphics_76,x:-19.8588,y:14.0674}).wait(1).to({graphics:mask_graphics_77,x:-19.0132,y:13.956}).wait(1).to({graphics:mask_graphics_78,x:-18.1675,y:13.8445}).wait(1).to({graphics:mask_graphics_79,x:-17.3216,y:13.7327}).wait(1).to({graphics:mask_graphics_80,x:-16.4756,y:13.6207}).wait(1).to({graphics:mask_graphics_81,x:-15.6295,y:13.5085}).wait(1).to({graphics:mask_graphics_82,x:-14.7832,y:13.3962}).wait(1).to({graphics:mask_graphics_83,x:-13.9369,y:13.2836}).wait(1).to({graphics:mask_graphics_84,x:-13.0904,y:13.1709}).wait(1).to({graphics:mask_graphics_85,x:-12.2438,y:13.0579}).wait(1).to({graphics:mask_graphics_86,x:-11.3971,y:12.9447}).wait(1).to({graphics:mask_graphics_87,x:-10.5502,y:12.8313}).wait(1).to({graphics:mask_graphics_88,x:-9.7033,y:12.7178}).wait(1).to({graphics:mask_graphics_89,x:-8.8562,y:12.604}).wait(1).to({graphics:mask_graphics_90,x:-8.009,y:12.49}).wait(1).to({graphics:mask_graphics_91,x:-7.1618,y:12.3758}).wait(1).to({graphics:mask_graphics_92,x:-6.3144,y:12.2614}).wait(1).to({graphics:mask_graphics_93,x:-5.4669,y:12.1467}).wait(1).to({graphics:mask_graphics_94,x:-4.6193,y:12.0319}).wait(1).to({graphics:mask_graphics_95,x:-3.7716,y:11.9169}).wait(1).to({graphics:mask_graphics_96,x:-2.9238,y:11.8016}).wait(1).to({graphics:mask_graphics_97,x:-2.0759,y:11.6861}).wait(1).to({graphics:mask_graphics_98,x:-1.2279,y:11.5704}).wait(1).to({graphics:mask_graphics_99,x:-0.3798,y:11.4545}).wait(1).to({graphics:mask_graphics_100,x:0.4684,y:11.3384}).wait(1).to({graphics:mask_graphics_101,x:1.3167,y:11.2221}).wait(1).to({graphics:mask_graphics_102,x:2.1651,y:11.1055}).wait(1).to({graphics:mask_graphics_103,x:3.0136,y:10.9887}).wait(1).to({graphics:mask_graphics_104,x:3.8621,y:10.8717}).wait(1).to({graphics:mask_graphics_105,x:4.7108,y:10.7545}).wait(1).to({graphics:mask_graphics_106,x:5.5595,y:10.637}).wait(1).to({graphics:mask_graphics_107,x:6.4083,y:10.5193}).wait(1).to({graphics:mask_graphics_108,x:7.2572,y:10.4014}).wait(1).to({graphics:mask_graphics_109,x:8.1062,y:10.2833}).wait(1).to({graphics:mask_graphics_110,x:8.9553,y:10.1649}).wait(1).to({graphics:mask_graphics_111,x:9.8044,y:10.0463}).wait(1).to({graphics:mask_graphics_112,x:10.6536,y:9.9274}).wait(1).to({graphics:mask_graphics_113,x:11.5029,y:9.8084}).wait(1).to({graphics:mask_graphics_114,x:12.3523,y:9.6891}).wait(1).to({graphics:mask_graphics_115,x:12.4672,y:9.5695}).wait(1).to({graphics:mask_graphics_116,x:12.2825,y:9.4497}).wait(1).to({graphics:mask_graphics_117,x:12.0794,y:9.3297}).wait(1).to({graphics:mask_graphics_118,x:11.8578,y:9.2095}).wait(1).to({graphics:mask_graphics_119,x:11.6178,y:9.089}).wait(1).to({graphics:mask_graphics_120,x:11.3594,y:8.9682}).wait(1).to({graphics:mask_graphics_121,x:11.0825,y:8.8472}).wait(1).to({graphics:mask_graphics_122,x:10.7873,y:8.726}).wait(1).to({graphics:mask_graphics_123,x:10.4736,y:8.6045}).wait(1).to({graphics:mask_graphics_124,x:10.1416,y:8.4828}).wait(1).to({graphics:mask_graphics_125,x:9.7911,y:8.3608}).wait(1).to({graphics:mask_graphics_126,x:38.5343,y:10.3537}).wait(99));

	// picoffort
	this.instance_3 = new lib.fortorange();
	this.instance_3.setTransform(-500,-202);

	var maskedShapeInstanceList = [this.instance_3];

	for(var shapedInstanceItr = 0; shapedInstanceItr < maskedShapeInstanceList.length; shapedInstanceItr++) {
		maskedShapeInstanceList[shapedInstanceItr].mask = mask;
	}

	this.timeline.addTween(cjs.Tween.get(this.instance_3).wait(225));

	// map
	this.instance_4 = new lib.fortorangealbanymap();
	this.instance_4.setTransform(-500,-202);

	this.instance_5 = new lib.FORTOframesize();
	this.instance_5.setTransform(-500,-202);

	this.shape = new cjs.Shape();
	this.shape.graphics.f().s("#FFFFFF").ss(1,1,1).p("EhOMgfUMCcZAAAMAAAA+pMicZAAAg");
	this.shape.setTransform(0.5,-1.5);

	this.shape_1 = new cjs.Shape();
	this.shape_1.graphics.f("#406991").s().p("EhOMAfVMAAAg+pMCcZAAAMAAAA+pg");
	this.shape_1.setTransform(0.5,-1.5);

	this.timeline.addTween(cjs.Tween.get({}).to({state:[{t:this.shape_1},{t:this.shape},{t:this.instance_5},{t:this.instance_4}]}).to({state:[{t:this.shape_1},{t:this.shape},{t:this.instance_5},{t:this.instance_4}]},165).wait(60));

	this._renderFirstFrame();

}).prototype = p = new cjs.MovieClip();
p.nominalBounds = new cjs.Rectangle(-501,-203,1003,403);


(lib.IceAges = function(mode,startPosition,loop,reversed) {
if (loop == null) { loop = false; }
if (reversed == null) { reversed = false; }
	var props = new Object();
	props.mode = mode;
	props.startPosition = startPosition;
	props.labels = {};
	props.loop = loop;
	props.reversed = reversed;
	cjs.MovieClip.apply(this,[props]);

	// timeline functions:
	this.frame_0 = function() {
		this.stop();
	}
	this.frame_188 = function() {
		this.getStage().getChildAt(0).changeSlide(slideNum);
	}

	// actions tween:
	this.timeline.addTween(cjs.Tween.get(this).call(this.frame_0).wait(188).call(this.frame_188).wait(22));

	// Layer 5
	this.instance = new lib.Tween22("synched",0);
	this.instance.setTransform(792.5,209);
	this.instance.alpha = 0;
	this.instance._off = true;

	this.timeline.addTween(cjs.Tween.get(this.instance).wait(56).to({_off:false},0).to({alpha:1},33).wait(121));

	// Layer 4 (mask)
	var mask = new cjs.Shape();
	mask._off = true;
	var mask_graphics_0 = new cjs.Graphics().p("EhOHAfQMAAAg+fMCcPAAAMAAAA+fg");
	var mask_graphics_209 = new cjs.Graphics().p("EhOHAfQMAAAg+fMCcPAAAMAAAA+fg");

	this.timeline.addTween(cjs.Tween.get(mask).to({graphics:mask_graphics_0,x:500,y:200}).wait(209).to({graphics:mask_graphics_209,x:500,y:200}).wait(1));

	// Layer 2
	this.instance_1 = new lib.Tween17("synched",0);
	this.instance_1.setTransform(455,196.5);

	var maskedShapeInstanceList = [this.instance_1];

	for(var shapedInstanceItr = 0; shapedInstanceItr < maskedShapeInstanceList.length; shapedInstanceItr++) {
		maskedShapeInstanceList[shapedInstanceItr].mask = mask;
	}

	this.timeline.addTween(cjs.Tween.get(this.instance_1).to({x:548.35,y:203.7},209,cjs.Ease.get(0.09)).wait(1));

	this._renderFirstFrame();

}).prototype = p = new cjs.MovieClip();
p.nominalBounds = new cjs.Rectangle(0,0,1000,400);


(lib.Captured = function(mode,startPosition,loop,reversed) {
if (loop == null) { loop = true; }
if (reversed == null) { reversed = false; }
	var props = new Object();
	props.mode = mode;
	props.startPosition = startPosition;
	props.labels = {};
	props.loop = loop;
	props.reversed = reversed;
	cjs.MovieClip.apply(this,[props]);

	// timeline functions:
	this.frame_0 = function() {
		this.stop();
	}
	this.frame_194 = function() {
		this.getStage().getChildAt(0).changeSlide(slideNum);
	}

	// actions tween:
	this.timeline.addTween(cjs.Tween.get(this).call(this.frame_0).wait(194).call(this.frame_194).wait(16));

	// Layer_4
	this.instance = new lib.Tween89("synched",0);
	this.instance.setTransform(727.75,180.85);
	this.instance.alpha = 0;
	this.instance._off = true;

	this.instance_1 = new lib.Tween90("synched",0);
	this.instance_1.setTransform(727.75,180.85);

	this.instance_2 = new lib.Asset201x();
	this.instance_2.setTransform(-301,-38);

	this.timeline.addTween(cjs.Tween.get({}).to({state:[]}).to({state:[{t:this.instance}]},24).to({state:[{t:this.instance_1}]},15).to({state:[{t:this.instance_2}]},170).wait(1));
	this.timeline.addTween(cjs.Tween.get(this.instance).wait(24).to({_off:false},0).to({_off:true,alpha:1},15).wait(171));

	// Layer_3
	this.instance_3 = new lib.Tween91("synched",0);
	this.instance_3.setTransform(252.55,200.55);
	this.instance_3.alpha = 0;
	this.instance_3._off = true;

	this.instance_4 = new lib.Tween92("synched",0);
	this.instance_4.setTransform(252.55,200.55);

	this.instance_5 = new lib.Asset131x();
	this.instance_5.setTransform(156,-54);

	this.timeline.addTween(cjs.Tween.get({}).to({state:[]}).to({state:[{t:this.instance_3}]},28).to({state:[{t:this.instance_4}]},14).to({state:[{t:this.instance_5}]},167).wait(1));
	this.timeline.addTween(cjs.Tween.get(this.instance_3).wait(28).to({_off:false},0).to({_off:true,alpha:1},14).wait(168));

	// Layer_2
	this.instance_6 = new lib.Tween87("synched",0);
	this.instance_6.setTransform(229,200,1,1,-9.6946);
	this.instance_6.alpha = 0;
	this.instance_6._off = true;

	this.instance_7 = new lib.Tween88("synched",0);
	this.instance_7.setTransform(250,200);

	this.instance_8 = new lib.capturedlobbylcd();
	this.instance_8.setTransform(300,1);

	this.timeline.addTween(cjs.Tween.get({}).to({state:[]}).to({state:[{t:this.instance_6}]},14).to({state:[{t:this.instance_7}]},16).to({state:[{t:this.instance_8}]},179).wait(1));
	this.timeline.addTween(cjs.Tween.get(this.instance_6).wait(14).to({_off:false},0).to({_off:true,rotation:0,x:250,alpha:1},16).wait(180));

	// Layer_1
	this.shape = new cjs.Shape();
	this.shape.graphics.f("#774541").s().p("EhOHAfaMAAAg+zMCcPAAAMAAAA+zg");
	this.shape.setTransform(500,201);

	this.timeline.addTween(cjs.Tween.get(this.shape).wait(210));

	this._renderFirstFrame();

}).prototype = p = new cjs.MovieClip();
p.nominalBounds = new cjs.Rectangle(-301,-54,1601,510);


(lib.bhm = function(mode,startPosition,loop,reversed) {
if (loop == null) { loop = true; }
if (reversed == null) { reversed = false; }
	var props = new Object();
	props.mode = mode;
	props.startPosition = startPosition;
	props.labels = {};
	props.loop = loop;
	props.reversed = reversed;
	cjs.MovieClip.apply(this,[props]);

	// timeline functions:
	this.frame_0 = function() {
		this.stop();''
	}
	this.frame_194 = function() {
		this.getStage().getChildAt(0).changeSlide(slideNum);
	}

	// actions tween:
	this.timeline.addTween(cjs.Tween.get(this).call(this.frame_0).wait(194).call(this.frame_194).wait(26));

	// Layer_7 (mask)
	var mask = new cjs.Shape();
	mask._off = true;
	var mask_graphics_12 = new cjs.Graphics().p("EhPsAVTIAAgoMCfZAAAIAAAog");
	var mask_graphics_13 = new cjs.Graphics().p("EhPsAVnIAAh9MCfZAAAIAAB9g");
	var mask_graphics_14 = new cjs.Graphics().p("EhPsAV8IAAjSMCfZAAAIAADSg");
	var mask_graphics_15 = new cjs.Graphics().p("EhPsAWQIAAknMCfZAAAIAAEng");
	var mask_graphics_16 = new cjs.Graphics().p("EhPsAWlIAAl8MCfZAAAIAAF8g");
	var mask_graphics_17 = new cjs.Graphics().p("EhPsAW5IAAnRMCfZAAAIAAHRg");
	var mask_graphics_18 = new cjs.Graphics().p("EhPsAXNIAAomMCfZAAAIAAImg");

	this.timeline.addTween(cjs.Tween.get(mask).to({graphics:null,x:0,y:0}).wait(12).to({graphics:mask_graphics_12,x:509.375,y:136.275}).wait(1).to({graphics:mask_graphics_13,x:509.375,y:138.3213}).wait(1).to({graphics:mask_graphics_14,x:509.375,y:140.3676}).wait(1).to({graphics:mask_graphics_15,x:509.375,y:142.4139}).wait(1).to({graphics:mask_graphics_16,x:509.375,y:144.4602}).wait(1).to({graphics:mask_graphics_17,x:509.375,y:146.5065}).wait(1).to({graphics:mask_graphics_18,x:509.375,y:148.525}).wait(202));

	// Layer_8
	this.shape = new cjs.Shape();
	this.shape.graphics.f("#FFFFFF").s().p("ABYD5IAAi9IiwAAIAAC9IiJAAIAAnxICJAAIAAC6ICwAAIAAi6ICKAAIAAHxg");
	this.shape.setTransform(925.975,269.05);

	this.shape_1 = new cjs.Shape();
	this.shape_1.graphics.f("#FFFFFF").s().p("AhED5IAAl3IiVAAIAAh6IGzAAIAAB6IiWAAIAAF3g");
	this.shape_1.setTransform(875.55,269.05);

	this.shape_2 = new cjs.Shape();
	this.shape_2.graphics.f("#FFFFFF").s().p("AByD5IjUkQIAAEQIiIAAIAAnxICBAAIDMEGIAAkGICIAAIAAHxg");
	this.shape_2.setTransform(824.275,269.05);

	this.shape_3 = new cjs.Shape();
	this.shape_3.graphics.f("#FFFFFF").s().p("");
	this.shape_3.setTransform(766.875,269.05);

	this.shape_4 = new cjs.Shape();
	this.shape_4.graphics.f("#FFFFFF").s().p("AB/D5IAAkdIh/DBIgBAAIh/i/IAAEbIiHAAIAAnxICRAAIB2DBIB3jBICRAAIAAHxg");
	this.shape_4.setTransform(706.625,269.05);

	this.shape_5 = new cjs.Shape();
	this.shape_5.graphics.f("#FFFFFF").s().p("AhED5IAAi6Ii9k3ICdAAIBlC1IBni1ICaAAIi9E1IAAC8g");
	this.shape_5.setTransform(630.525,269.05);

	this.shape_6 = new cjs.Shape();
	this.shape_6.graphics.f("#FFFFFF").s().p("ABFD5IhjiXIgBAAIg7AAIAACXIiKAAIAAnxIDqAAQA6AAAnAPQAoAOAZAZQAsArAABJIAAABQAAA6gbAlQgcAlgvAUIB3CugAhagJIBaAAQAjAAAUgQQATgQAAgbIAAgCQAAgdgUgPQgUgOgiAAIhaAAg");
	this.shape_6.setTransform(581.125,269.05);

	this.shape_7 = new cjs.Shape();
	this.shape_7.graphics.f("#FFFFFF").s().p("");
	this.shape_7.setTransform(524.325,269.05);

	this.shape_8 = new cjs.Shape();
	this.shape_8.graphics.f("#FFFFFF").s().p("AhED5IAAl3IiVAAIAAh6IGzAAIAAB6IiWAAIAAF3g");
	this.shape_8.setTransform(471.45,269.05);

	this.shape_9 = new cjs.Shape();
	this.shape_9.graphics.f("#FFFFFF").s().p("");
	this.shape_9.setTransform(424.075,269.05);

	this.shape_10 = new cjs.Shape();
	this.shape_10.graphics.f("#FFFFFF").s().p("AhED5IAAnxICJAAIAAHxg");
	this.shape_10.setTransform(390.175,269.05);

	this.shape_11 = new cjs.Shape();
	this.shape_11.graphics.f("#FFFFFF").s().p("ABYD5IAAi9IiwAAIAAC9IiJAAIAAnxICJAAIAAC6ICwAAIAAi6ICKAAIAAHxg");
	this.shape_11.setTransform(351.575,269.05);

	this.shape_12 = new cjs.Shape();
	this.shape_12.graphics.f("#FFFFFF").s().p("ABKD5Ih8i5IgzA4IAACBIiJAAIAAnxICJAAIAADKICrjKICjAAIi8DWIDCEbg");
	this.shape_12.setTransform(281.425,269.05);

	this.shape_13 = new cjs.Shape();
	this.shape_13.graphics.f("#FFFFFF").s().p("");
	this.shape_13.setTransform(227.825,269.05);

	this.shape_14 = new cjs.Shape();
	this.shape_14.graphics.f("#FFFFFF").s().p("ACCD6IgkhYIi/AAIgjBYIiRAAIDUnzICEAAIDTHzgAA3A3Ig3iMIg4CMIBvAAg");
	this.shape_14.setTransform(173.1,268.875);

	this.shape_15 = new cjs.Shape();
	this.shape_15.graphics.f("#FFFFFF").s().p("Ai8D5IAAnxICJAAIAAF4IDwAAIAAB5g");
	this.shape_15.setTransform(124.525,269.05);

	this.shape_16 = new cjs.Shape();
	this.shape_16.graphics.f("#FFFFFF").s().p("AjdD5IAAnxID9AAQAvAAAhAMQAiALAVAWQAPAPAJAUQAIAUAAAZIAAACQAAArgVAbQgWAbghAOQAuANAbAcQAaAdAAAyIAAABQAABBgxAlQgyAjhZABgAhXCKIBpAAQAgAAASgMQARgLAAgXIAAgBQAAgVgRgMQgQgMgjAAIhoAAgAhXgyIBXAAQAfAAAQgKQARgLAAgVIAAgCQAAgVgQgKQgPgLggAAIhYAAg");
	this.shape_16.setTransform(77.075,269.05);

	var maskedShapeInstanceList = [this.shape,this.shape_1,this.shape_2,this.shape_3,this.shape_4,this.shape_5,this.shape_6,this.shape_7,this.shape_8,this.shape_9,this.shape_10,this.shape_11,this.shape_12,this.shape_13,this.shape_14,this.shape_15,this.shape_16];

	for(var shapedInstanceItr = 0; shapedInstanceItr < maskedShapeInstanceList.length; shapedInstanceItr++) {
		maskedShapeInstanceList[shapedInstanceItr].mask = mask;
	}

	this.timeline.addTween(cjs.Tween.get({}).to({state:[]}).to({state:[{t:this.shape_16},{t:this.shape_15},{t:this.shape_14},{t:this.shape_13},{t:this.shape_12},{t:this.shape_11},{t:this.shape_10},{t:this.shape_9},{t:this.shape_8},{t:this.shape_7},{t:this.shape_6},{t:this.shape_5},{t:this.shape_4},{t:this.shape_3},{t:this.shape_2},{t:this.shape_1},{t:this.shape}]},12).wait(208));

	// Layer_10
	this.shape_17 = new cjs.Shape();
	this.shape_17.graphics.f("#000000").s().p("EiFHAKjIAA1FMEKOAAAIAAVFg");
	this.shape_17.setTransform(480,278.5);
	this.shape_17._off = true;

	var maskedShapeInstanceList = [this.shape_17];

	for(var shapedInstanceItr = 0; shapedInstanceItr < maskedShapeInstanceList.length; shapedInstanceItr++) {
		maskedShapeInstanceList[shapedInstanceItr].mask = mask;
	}

	this.timeline.addTween(cjs.Tween.get(this.shape_17).wait(12).to({_off:false},0).wait(208));

	// Layer_13
	this.instance = new lib.Tween104("synched",0);
	this.instance.setTransform(297,200);
	this.instance.alpha = 0;
	this.instance._off = true;

	this.instance_1 = new lib.Tween105("synched",0);
	this.instance_1.setTransform(357,200);

	this.timeline.addTween(cjs.Tween.get({}).to({state:[]}).to({state:[{t:this.instance}]},24).to({state:[{t:this.instance_1}]},9).wait(187));
	this.timeline.addTween(cjs.Tween.get(this.instance).wait(24).to({_off:false},0).to({_off:true,x:357,alpha:1},9).wait(187));

	// Layer_14
	this.instance_2 = new lib.Tween106("synched",0);
	this.instance_2.setTransform(587,200);
	this.instance_2.alpha = 0;
	this.instance_2._off = true;

	this.instance_3 = new lib.Tween107("synched",0);
	this.instance_3.setTransform(617,200);

	this.timeline.addTween(cjs.Tween.get({}).to({state:[]}).to({state:[{t:this.instance_2}]},29).to({state:[{t:this.instance_3}]},10).wait(181));
	this.timeline.addTween(cjs.Tween.get(this.instance_2).wait(29).to({_off:false},0).to({_off:true,x:617,alpha:1},10).wait(181));

	// Layer_15
	this.instance_4 = new lib.Tween108("synched",0);
	this.instance_4.setTransform(846,200);
	this.instance_4.alpha = 0;
	this.instance_4._off = true;

	this.instance_5 = new lib.Tween109("synched",0);
	this.instance_5.setTransform(866,200);

	this.timeline.addTween(cjs.Tween.get({}).to({state:[]}).to({state:[{t:this.instance_4}]},34).to({state:[{t:this.instance_5}]},10).wait(176));
	this.timeline.addTween(cjs.Tween.get(this.instance_4).wait(34).to({_off:false},0).to({_off:true,x:866,alpha:1},10).wait(176));

	// Layer_12
	this.instance_6 = new lib.Tween102("synched",0);
	this.instance_6.setTransform(71.5,200);
	this.instance_6.alpha = 0.5313;
	this.instance_6._off = true;

	this.instance_7 = new lib.Tween103("synched",0);
	this.instance_7.setTransform(92.5,200);

	this.timeline.addTween(cjs.Tween.get({}).to({state:[]}).to({state:[{t:this.instance_6}]},19).to({state:[{t:this.instance_7}]},9).wait(192));
	this.timeline.addTween(cjs.Tween.get(this.instance_6).wait(19).to({_off:false},0).to({_off:true,x:92.5,alpha:1},9).wait(192));

	// Layer_5
	this.instance_8 = new lib.Tween95("synched",0);
	this.instance_8.setTransform(512.8,205.75,2.1069,1,0,0,0,0.1,0);

	this.instance_9 = new lib.Tween96("synched",0);
	this.instance_9.setTransform(1249.4,205.95);

	this.timeline.addTween(cjs.Tween.get({}).to({state:[{t:this.instance_8}]}).to({state:[{t:this.instance_9}]},13).wait(207));
	this.timeline.addTween(cjs.Tween.get(this.instance_8).to({_off:true,regX:0,scaleX:1,x:1249.4,y:205.95},13).wait(207));

	// Layer_3
	this.shape_18 = new cjs.Shape();
	this.shape_18.graphics.f("#FFFFFF").s().p("");
	this.shape_18.setTransform(501.025,202.5);

	this.timeline.addTween(cjs.Tween.get(this.shape_18).wait(220));

	// Layer_2
	this.shape_19 = new cjs.Shape();
	this.shape_19.graphics.f("#000000").s().p("EhOHAfQMAAAg+fMCcPAAAMAAAA+fg");
	this.shape_19.setTransform(500,200);

	this.timeline.addTween(cjs.Tween.get(this.shape_19).wait(220));

	this._renderFirstFrame();

}).prototype = p = new cjs.MovieClip();
p.nominalBounds = new cjs.Rectangle(-104,-17,1600.1,439);


// stage content:
(lib.nysm_slider = function(mode,startPosition,loop,reversed) {
if (loop == null) { loop = false; }
if (reversed == null) { reversed = false; }
	var props = new Object();
	props.mode = mode;
	props.startPosition = startPosition;
	props.labels = {};
	props.loop = loop;
	props.reversed = reversed;
	cjs.MovieClip.apply(this,[props]);

	this.actionFrames = [0,1];
	// timeline functions:
	this.frame_0 = function() {
		window.firstSlide = 1;
		window.numSlides = 4;
		var self = this;
		window.slideNum = firstSlide;
		//var curSlide = "self.symbol1";
		this.changeSlide = function () {
			
			var nextSlide;
			var self = this;
			var curSlide = self["symbol" + slideNum];
			console.log(curSlide);
			if (slideNum == numSlides) {
				var nextSlide = self["symbol1"];
				window.slideNum = 1;
				console.log(slideNum);
			} else {
				window.slideNum++;
				console.log(slideNum);
				var nextSlide = self["symbol" + slideNum];
			}
			//	createjs.Tween.get(curSlide).to({alpha: 0}, 750);
			console.log('the curslide is ' + curSlide);
			console.log('the nextslide is ' + nextSlide);
				nextSlide.alpha = 0;
				nextSlide.visible = true;
				createjs.Tween.get(nextSlide).to({alpha: 1}, 750);
				nextSlide.gotoAndPlay(1);
				curSlide.visible = false;
			
		}
	}
	this.frame_1 = function() {
		var self = this;
		self["symbol" + firstSlide].alpha = 0;
		self["symbol" + firstSlide].visible = true;
		createjs.Tween.get(self["symbol" + firstSlide]).to({alpha: 1}, 500);
		var frequency = 3;
		stage.enableMouseOver(frequency);
		self["symbol" + firstSlide].gotoAndPlay(0);
		
		/* Click to Go to Web Page
		Clicking on the specified symbol instance loads the URL in a new browser window.
		
		Instructions:
		1. Replace http://www.adobe.com with the desired URL address.
		   Keep the quotation marks ("").
		*/
		
				this.symbol4.addEventListener("click", fl_ClickToGoToWebPage4);
				this.symbol3.addEventListener("click", fl_ClickToGoToWebPage3);
				this.symbol2.addEventListener("click", fl_ClickToGoToWebPage2);
				this.symbol1.addEventListener("click", fl_ClickToGoToWebPage1);
		
				
				function fl_ClickToGoToWebPage2() {
					window.open("exhibitions/captured-recent-photography-acquisitions", "_self");
				}
				
				
				function fl_ClickToGoToWebPage3() {
					window.open("exhibitions/ice-ages", "_self");
				}
				
				
					function fl_ClickToGoToWebPage4() {
					window.open("exhibitions/ongoing/fort-orange", "_self");
				}
						function fl_ClickToGoToWebPage1() {
					window.open("black-history-month", "_self");
				}
	}

	// actions tween:
	this.timeline.addTween(cjs.Tween.get(this).call(this.frame_0).wait(1).call(this.frame_1).wait(1));

	// bhm
	this.symbol1 = new lib.bhm();
	this.symbol1.name = "symbol1";
	this.symbol1.setTransform(500,200,1,1,0,0,0,500,200);
	this.symbol1.visible = false;

	this.timeline.addTween(cjs.Tween.get(this.symbol1).wait(2));

	// captured
	this.symbol2 = new lib.Captured();
	this.symbol2.name = "symbol2";
	this.symbol2.setTransform(500,200,1,1,0,0,0,500,201);
	this.symbol2.visible = false;

	this.timeline.addTween(cjs.Tween.get(this.symbol2).wait(2));

	// iceages
	this.symbol3 = new lib.IceAges();
	this.symbol3.name = "symbol3";
	this.symbol3.setTransform(500,200,1,1,0,0,0,500,200);
	this.symbol3.visible = false;

	this.timeline.addTween(cjs.Tween.get(this.symbol3).wait(2));

	// fortorange
	this.symbol4 = new lib.votesforwomen();
	this.symbol4.name = "symbol4";
	this.symbol4.setTransform(1000,401,1,1,0,0,0,500,200);
	this.symbol4.visible = false;

	this.timeline.addTween(cjs.Tween.get(this.symbol4).wait(2));

	// open
	this.symbol5 = new lib.wereopen();
	this.symbol5.name = "symbol5";
	this.symbol5.setTransform(501,201,1,1,0,0,0,500,201);
	this.symbol5.visible = false;

	this.timeline.addTween(cjs.Tween.get(this.symbol5).wait(2));

	this._renderFirstFrame();

}).prototype = p = new lib.AnMovieClip();
p.nominalBounds = new cjs.Rectangle(405,183,627.3,239);
// library properties:
lib.properties = {
	id: '5222455380EF4C3C8D1B33262CFD3544',
	width: 1000,
	height: 400,
	fps: 24,
	color: "#FFFFFF",
	opacity: 1.00,
	manifest: [
		{src:"/sites/all/themes/nysm/homepageanimation/images/Asset131x.png", id:"Asset131x"},
		{src:"/sites/all/themes/nysm/homepageanimation/images/Asset201x.png", id:"Asset201x"},
		{src:"/sites/all/themes/nysm/homepageanimation/images/bluefacade.jpg", id:"bluefacade"},
		{src:"/sites/all/themes/nysm/homepageanimation/images/BorealBog_Mural_SAMPLEkg.jpg", id:"BorealBog_Mural_SAMPLEkg"},
		{src:"/sites/all/themes/nysm/homepageanimation/images/capturedlobbylcd.png", id:"capturedlobbylcd"},
		{src:"/sites/all/themes/nysm/homepageanimation/images/circle.png", id:"circle"},
		{src:"/sites/all/themes/nysm/homepageanimation/images/douglas.jpg", id:"douglas"},
		{src:"/sites/all/themes/nysm/homepageanimation/images/FORTOframesize.jpg", id:"FORTOframesize"},
		{src:"/sites/all/themes/nysm/homepageanimation/images/fortorangealbanymap.jpg", id:"fortorangealbanymap"},
		{src:"/sites/all/themes/nysm/homepageanimation/images/fortorange.jpg", id:"fortorange"},
		{src:"/sites/all/themes/nysm/homepageanimation/images/henryjohnson.jpg", id:"henryjohnson"},
		{src:"/sites/all/themes/nysm/homepageanimation/images/iceagestitle.png", id:"iceagestitle"},
		{src:"/sites/all/themes/nysm/homepageanimation/images/shirley.jpg", id:"shirley"},
		{src:"/sites/all/themes/nysm/homepageanimation/images/title01.png", id:"title01"},
		{src:"/sites/all/themes/nysm/homepageanimation/images/title02.png", id:"title02"},
		{src:"/sites/all/themes/nysm/homepageanimation/images/tubman.jpg", id:"tubman"},
		{src:"/sites/all/themes/nysm/homepageanimation/images/underline.png", id:"underline"},
		{src:"/sites/all/themes/nysm/homepageanimation/images/wereopentextpngcopy.png", id:"wereopentextpngcopy"}
	],
	preloads: []
};



// bootstrap callback support:

(lib.Stage = function(canvas) {
	createjs.Stage.call(this, canvas);
}).prototype = p = new createjs.Stage();

p.setAutoPlay = function(autoPlay) {
	this.tickEnabled = autoPlay;
}
p.play = function() { this.tickEnabled = true; this.getChildAt(0).gotoAndPlay(this.getTimelinePosition()) }
p.stop = function(ms) { if(ms) this.seek(ms); this.tickEnabled = false; }
p.seek = function(ms) { this.tickEnabled = true; this.getChildAt(0).gotoAndStop(lib.properties.fps * ms / 1000); }
p.getDuration = function() { return this.getChildAt(0).totalFrames / lib.properties.fps * 1000; }

p.getTimelinePosition = function() { return this.getChildAt(0).currentFrame / lib.properties.fps * 1000; }

an.bootcompsLoaded = an.bootcompsLoaded || [];
if(!an.bootstrapListeners) {
	an.bootstrapListeners=[];
}

an.bootstrapCallback=function(fnCallback) {
	an.bootstrapListeners.push(fnCallback);
	if(an.bootcompsLoaded.length > 0) {
		for(var i=0; i<an.bootcompsLoaded.length; ++i) {
			fnCallback(an.bootcompsLoaded[i]);
		}
	}
};

an.compositions = an.compositions || {};
an.compositions['5222455380EF4C3C8D1B33262CFD3544'] = {
	getStage: function() { return exportRoot.stage; },
	getLibrary: function() { return lib; },
	getSpriteSheet: function() { return ss; },
	getImages: function() { return img; }
};

an.compositionLoaded = function(id) {
	an.bootcompsLoaded.push(id);
	for(var j=0; j<an.bootstrapListeners.length; j++) {
		an.bootstrapListeners[j](id);
	}
}

an.getComposition = function(id) {
	return an.compositions[id];
}


an.makeResponsive = function(isResp, respDim, isScale, scaleType, domContainers) {		
	var lastW, lastH, lastS=1;		
	window.addEventListener('resize', resizeCanvas);		
	resizeCanvas();		
	function resizeCanvas() {			
		var w = lib.properties.width, h = lib.properties.height;			
		var iw = window.innerWidth, ih=window.innerHeight;			
		var pRatio = window.devicePixelRatio || 1, xRatio=iw/w, yRatio=ih/h, sRatio=1;			
		if(isResp) {                
			if((respDim=='width'&&lastW==iw) || (respDim=='height'&&lastH==ih)) {                    
				sRatio = lastS;                
			}				
			else if(!isScale) {					
				if(iw<w || ih<h)						
					sRatio = Math.min(xRatio, yRatio);				
			}				
			else if(scaleType==1) {					
				sRatio = Math.min(xRatio, yRatio);				
			}				
			else if(scaleType==2) {					
				sRatio = Math.max(xRatio, yRatio);				
			}			
		}
		domContainers[0].width = w * pRatio * sRatio;			
		domContainers[0].height = h * pRatio * sRatio;
		domContainers.forEach(function(container) {				
			container.style.width = w * sRatio + 'px';				
			container.style.height = h * sRatio + 'px';			
		});
		stage.scaleX = pRatio*sRatio;			
		stage.scaleY = pRatio*sRatio;
		lastW = iw; lastH = ih; lastS = sRatio;            
		stage.tickOnUpdate = false;            
		stage.update();            
		stage.tickOnUpdate = true;		
	}
}
an.handleSoundStreamOnTick = function(event) {
	if(!event.paused){
		var stageChild = stage.getChildAt(0);
		if(!stageChild.paused || stageChild.ignorePause){
			stageChild.syncStreamSounds();
		}
	}
}
an.handleFilterCache = function(event) {
	if(!event.paused){
		var target = event.target;
		if(target){
			if(target.filterCacheList){
				for(var index = 0; index < target.filterCacheList.length ; index++){
					var cacheInst = target.filterCacheList[index];
					if((cacheInst.startFrame <= target.currentFrame) && (target.currentFrame <= cacheInst.endFrame)){
						cacheInst.instance.cache(cacheInst.x, cacheInst.y, cacheInst.w, cacheInst.h);
					}
				}
			}
		}
	}
}


})(createjs = createjs||{}, AdobeAn = AdobeAn||{});
var createjs, AdobeAn;